...
import type { GitHubRepoResponse, ProjectSummary } from "~/types";
...
export const fetchGitHubProjects = async (
	...
	if (import.meta.env.DEV) {
		console.log(
			`🔄 Fetching fresh GitHub projects for ${repoUrls.length} repositories`,
		);
	}
				...
				if (data.languages_url) {
					try {
						...
						if (languagesResponse.ok) {
							const languageData = (await languagesResponse.json()) as Record<
								string,
								number
							>;
						...
						if (import.meta.env.DEV) {
							console.warn(
								`Failed to load languages for ${owner}/${repo}`,
								languagesError,
							);
						}
					}
